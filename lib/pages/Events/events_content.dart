...
import 'package:folk/pages/Events/full_event_screen.dart';
            ...
            child: PageView(
              ...
              children: <Widget>[
                Consumer<EventProvider>(
                  ...
                  List<EventModel> _listEvents = eventProvider.listEvents;
                      ...
                      child: _listEvents.isNotEmpty
                          ? ListView.builder(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                EventModel _event = _listEvents[i];

                                return EventCard(
                                  EventImage(
                                    "${Constants.USERS_POSTS_IMAGES}" +
                                        "${_listEvents[i].postImg}",
                                    "${_listEvents[i].eventDate}",
                                    "Jan",
                                    Tag(
                                      initParticipateStatus(
                                          _listEvents[i].joinedCount,
                                          _listEvents[i].maxParticipantCount),
                                      Colors.white,
                                      Colors.deepOrange,
                                      Colors.transparent,
                                      fontSize: 8,
...
class EventCard extends StatelessWidget {
  ...
  EventCard(this.eventImage, this._eventModel, {this.displayTimeBar = false});
  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () {
        Provider.of<EventProvider>(context, listen: false).eventModel = _eventModel;
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => FullEventScreen()),
        );
      },
      child: Container(
        ...
        child: Column(
          children: <Widget>[
            (displayTimeBar) ? TimeBar(_eventModel) : Container(),
            EventDetailRow(eventImage, _eventModel),
          ],
        ),
      ),
    );
